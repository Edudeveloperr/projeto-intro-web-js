...
const parcelarCurso = () => {
    ...
    // desconto por número de parcelas
    if (nParcelas < 3 && nParcelas > 0) {
        desconto += (valorPagar * 20 / 100)
        textoDesconto += textoDesconto != "" ? `<br/>`+"concedido + 20% de desconto" : "será concedido 20% de desconto"
    }
    valorPagar -= desconto
    ...
    const result2 = `Valor total sem desconto: ${moedaBrasil(valorTotal,true)} <p></p>
